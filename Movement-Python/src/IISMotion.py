...
class IISMotion:
    ...
    def stepAllCollections(self, newDay):
        for key, collection in self.movableCollectionsSet.items(): #probably return all object that can be moved
            if (collection.ableOfMovement):
    ...
    def sendUpdateToFrontend(self):
        if (self.guiEnabled):
            ...
            if (len(features) > 0):
                json_data = json.dumps(data) #make python dic. to jason dic.
