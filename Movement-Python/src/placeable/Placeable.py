...
class Placeable:
    ...
    def __init__(self, locationsTable, map:Map, name):
        uid = UniqueID()
        self.id = uid.getId()
        self.name = name
        self.map = map
    ...
    def getGeoStruct(self):
        ...
        '''
        data = {}
        data["id"] = self.id #IMPORTANT here is that id, its actually lower but whatever
        data["type"] = "Feature"

        properties = {}
        properties["type"] = self.__class__.__name__
        properties["name"] = self.name #changed id -> name
